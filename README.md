...
1. Финолог видит транзакцию — срабатывает триггер в Запире
2. Далее запир пытается понять что он разбивает — ищет приложение и его даты.
3. В момент , когда человек заполняет форму — зап обрабатывает его данные и вложенный сабзап считает его ставку за указанный им приложении период. Чтобы ставка посчиталась — в ворклогалке должны быть вокрлоги автора за указанный в приложении промежуток времени. 
4. Затем зап отдает команду на сплит. 
5. Эта команда Запускает код, который берет данные с сервера.

<b>Разбиение конкретной ФОТ транзакции по репорт дейт:</b>

1. Выполняется, когда в канал отправляем команду - рзбтт айди транзакции, которую нужно разбить (рзбтт 43629927)
...
3. Далее зап отдает команду на сплит
4. Эта команда запускает код, который берет данные с сервера ворклога.

<b>Сплитилка для ставочников:</b>

1. Выполняется, когда в канал отправляем айди транзакции и через запятую перечисляем проекты, над которыми работал исполнитель (47811615 BLOG-229,BLOG-234,WBT-1355,WBT-1409)
2. Запир получает эту команду и отдает команду на сплит
