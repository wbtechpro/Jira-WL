...
* На [главной странице админ-панели](https://jira-wl.wbtech.pro/jira-client-admin/) в приложении Client создан пункт [«! Период, за которые необходимо загрузить ворклоги»](https://jira-wl.wbtech.pro/jira-client-admin/client/daysfordownloadmodel/worklogs-download-period/).

* При нажатии на пункт открывается страница с двумя полями ввода.
...
### Автоматическое обновление даты и количества дней 28-ого числа каждого месяца

* Этот функционал нужен для того, чтобы ежемесячно загружать ворклоги с 28 числа предыдущего месяца по 27 число текущего месяца включительно.

* Ночью 28-ого числа каждого месяца происходит автоматическое обновление или установка (если прежде поля были пустыми) конечной даты и количества дней для загрузки ворклогов:
  * Конечная дата - текущая, то есть 28 число текущего месяца и года;
  * Количество дней - каждый раз разное. Зависит от того, сколько дней в текущем календарном месяце (к примеру, 31 в январе, 28 в феврале).

* После обновления даты и количества дней ночью 28-ого числа загружаются ворклоги за указанный период.


### Автоматическое обновление даты каждый вторник

* Этот функционал нужен для того, чтобы каждый вторник загружались ворклоги за предшествующий период.

* Команда не работает в том случае, если вторник выпал на 28 число - тогда срабатывает команда для загрузки ворклогов за предыдущий месяц (см. выше).

* Количество дней, за которое загружаются ворклоги, неизменно - 90 дней.

* Ночью каждого вторника происходит автоматическое обновление конечной даты:
  * Конечная дата - текущая, то есть вторник текущей недели.

* После обновления конечной даты этой же ночью загружаются ворклоги за указанный период.

* Ворклоги текущего вторника не загружаются. Последним днем при загрузке ворклогов будет предыдуший день, то есть понедельник.
...
* Для того, чтобы передать аргументы команде run_client вручную, необходимо указать --end_date {YYYY-MM-DD} и/или --days_before {кол-во дней} соответственно.
